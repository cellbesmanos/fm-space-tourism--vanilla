...
function changeActivePanel(activeTab) {
  ...
  const mainContainer = document.querySelector("main.grid-container--crew");
...
function changeActivePicture(nextActivePanel) {
  const crewPictures = document.querySelectorAll("picture.crew-image");
  const activePicture = document.querySelector(
    `picture.crew-image[data-image-for='${nextActivePanel}']`
  );

  crewPictures.forEach((picture) => {
